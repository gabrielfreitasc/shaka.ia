...
'''`.trim()
  ...
  await prisma.prompt.create({
    data: {
      tile: 'Resumo',
      template: `Seu papel é gerar um resumo do vídeo ou áudio.
...
'''`.trim()
  ...
  await prisma.prompt.create({
    data: {
      tile: 'Transcrever',
      template: `Seu papel é gerar uma transcrição do vídeo ou áudio.

Abaixo você receberá uma transcrição desse vídeo, use essa transcrição para retorna-lá o que foi dito no vídeo ou áudio.
